...
def _int_to_text(num):
    ...
    cache = _int_to_text.__dict__.setdefault("_cache", {})
    if num in cache:
        return cache[num]
    ...
    if num < 20:
        ...
        result = tens[ten] + (" " + ones[one] if one != 0 else "")
    elif num < 1000:
        hundred, rem = divmod(num, 100)
        result = ones[hundred] + " hundred" + (" " + _int_to_text(rem) if rem != 0 else "")
    elif num < 1000000:
        thousand, rem = divmod(num, 1000)
        result = _int_to_text(thousand) + " thousand" + (" " + _int_to_text(rem) if rem != 0 else "")
    elif num < 1000000000:
        million, rem = divmod(num, 1000000)
        result = _int_to_text(million) + " million" + (" " + _int_to_text(rem) if rem != 0 else "")
    else:
        billion, rem = divmod(num, 1000000000)
        result = _int_to_text(billion) + " billion" + (" " + _int_to_text(rem) if rem != 0 else "")
